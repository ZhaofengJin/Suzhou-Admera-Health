...
#Translate the descriptions for the drugs and genes 
drug_transl_list={}
...
def myFirstPage(canvas,doc):
    ...
    canvas.drawImage(scriptFolder+"/img/header4.png",133*mm,257*mm,53*mm,4.5*mm)
...
class NumberedCanvas(canvas.Canvas):
    ...
    #draw the page numbers and company information in the page footer
...
def ParseArg():
    ...
    p.add_argument("-T","--cType",type=str,help="Type of Cancer",default="")
    if len(sys.argv)==1:
...
#input is the sample ID, file is the interperation file 
input = args.sID
InputFile = args.input
age = args.age.rstrip()
action_file = codecs.open(InputFile,'r',encoding = 'utf-8')

#get the data from the interperation file 
result, Genes = ParseActionFile(action_file)
action_file.close()
summary_File = codecs.open("%s/%s_summary.txt"%(args.folder,input),'w')
summary_Data=[input]
...
data = [
        ["客户信息"],
        ["姓名",args.pName, "年龄",age ,"病理诊断",args.cName],
        ["项目信息"],
...
for l in action_file.read().split("\n"):
    ...
    if group==4:
        ...
        if lsep[2]=="Nucleotide":
            ...
            TempData.append(lsep[3])
            #add the alteration into the summary file 
            if !((lsep[1]+"-"++lsep[3]) in summary_Data):
                summary_Data.append(lsep[1]+"-"++lsep[3])
        elif lsep[2]=="Variation Type":
            type=lsep[3]
            ...
            if type=="Fusion":
                ...
                for j in action_fileF.read().split("\n"):
                    if j.startswith("ALK"):
                        continue
                    jsep = j.split("\t")
                    #if the input is in the fusion file,record the corresponding value  
                    if input in jsep[0]:
                        if TempData[0]=="ALK":
                ...
                for j in action_fileP.read().split("\n"):
                    ...
                    jsep = j.split("\t")
                    if jsep[0]==TempData[0] and jsep[5]==TempData[3]:
                        #transform the data into percentage form
            ...
            TempData=[]
summary_File.write('\t'.join(summary_Data))
summary_File.close()
#点突变表格
...
if len(DataForPoint)>0:#get rid of the no data type 
...
#Insert the result of the gene test into the index page
...
#judge whether the data in the two tables is empty or not
...
#sort the data in the two tables 
...
if DataForIncrease!=[] and len(DataForIncrease[0])>1:
    for gene in (result["ClinBenefitsSame"]+result["ClinBenefitsDiff"]):
        if i>0 and (DataForIncrease[i][0]==DataForIncrease[i-1][0]) and (DataForIncrease[i][1]==DataForIncrease[i-1][1]):
            #if it is the start line or the data is the same as the previous line,skip this line of data
            i+=1
            continue
        for j in range(len(DataForIncrease)-i):
            # i is the start position of the gene name, l is the length
...
if (len(DataForPoint)+len(DataForID)+len(DataForAmp)+len(DataForAmp))==0 and args.cType=="T":
    Page_Third.append((Paragraph("<font size=13 name='hei'><b><br />*由于未检测到KRAS/NRAS/BRAF/ PI3KCA等癌症驱动基因突变，预计不降低肿瘤细胞对EGFR抗体的敏感性，包括西妥昔单抗、帕尼单抗。[NCCN]</b> </font>", body)))
...
#record the translation of the description into the dictionary data type 
GeneInfo_Dic = {}
Prevelence_Dic = {}
...
for GeneInfo in result["GeneInfo"]:
    ...
    for i in range(len(jsep)):
        ...
        if "amplification" in jsep[i]:
            Alteration = jsep[i][0:(jsep[i].find("amplification"))]
            disease = translate(jsep[i][(jsep[i].find("in")+2):jsep[i].find(":")])
            fre = jsep[i][(jsep[i].find(":")+1):len(jsep[i])]
            if i==0:
                sentence = Alteration + "在".decode("utf-8")+disease+"中的基因扩增频率为".decode("utf-8")+fre
            else:
                sentence = sentence + "<br />" + Alteration + "在".decode("utf-8")+disease+"中的基因扩增频率为".decode("utf-8")+fre
        
        if ("Fusion" in jsep[i]):
            Alteration = jsep[i][0:(jsep[i].find("Fusion"))]
            disease = translate(jsep[i][(jsep[i].find("in")+2):jsep[i].find(":")])
            fre = jsep[i][(jsep[i].find(":")+1):len(jsep[i])]
            if i==0:
                sentence = Alteration + "在".decode("utf-8")+disease+"中的基因融合频率为".decode("utf-8")+fre
            else:
                sentence = sentence+"<br />"+Alteration + "在".decode("utf-8")+disease+"中的基因融合频率为".decode("utf-8")+fre
            
        if ("fusion" in jsep[i]):
            Alteration = jsep[i][0:(jsep[i].find("fusion"))]
            disease = translate(jsep[i][(jsep[i].find("in")+2):jsep[i].find(":")])
            fre = jsep[i][(jsep[i].find(":")+1):len(jsep[i])]
            ge = jsep[i][(jsep[i].find("all")+4):(jsep[i].find("fusions")-1)]
            if i==0:
                sentence = Alteration + "在".decode("utf-8")+disease+"的所有".decode("utf-8")+ge+"基因融合中的发生率为".decode("utf-8")+fre
            else:
                sentence = sentence+"<br />"+Alteration + "在".decode("utf-8")+disease+"的所有".decode("utf-8")+ge+"基因融合中的发生率为".decode("utf-8")+fre
...
#by using the rstrip function to cut the space at the end of the line
Info_file = open(scriptFolder+"/OncoGxSelectV2_Panel_info_CN2.txt",'r')
Data_info = [x.rstrip().split("\t") for x in Info_file.read().split("\n") if len(x.split("\t"))>1]
...
#combine the same gene name and the drug name from the sites_and_drugs file
l = 0
...
print("Complete generating File %s"%args.outputPDF)